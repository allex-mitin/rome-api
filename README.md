...
branding:
  ...
  logo: /branding/logo.svg            # любой URL; без него — /logo.svg из сборки
...
откатится к логотипу из сборки (`/assets/logo.svg`), а не покажет битую картинку.

Логотип и иконка вкладки лежат в сборке не data-URI и не с хэшем в имени, а обычными файлами
`assets/logo.svg` и `assets/favicon.ico`. Оба можно заменить в развёрнутой сборке, не пересобирая
фронтенд; ключи `logo` и `favicon` в `settings.yml` переопределяют их на любой URL (относительный
разрешается от текущей страницы).

Брендинг — обычная статика, но её меняют отдельно от сборки, поэтому в
[`deploy/nginx.conf.example`](deploy/nginx.conf.example) каталог `/branding/`, `settings.js`,
`assets/favicon.ico` и `assets/logo.svg` исключены из правила «immutable-кэш на год» (без этого
новый логотип или иконка не показались бы до истечения года кэша).
