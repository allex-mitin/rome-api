...
- [x] **Второй проход по оболочке: спека больше не «в окне».** По требованию владельца убрана белая
      ...
      создаёт границу, без линии. Переключатель OpenAPI/AsyncAPI уменьшен до `dimension="s"`, имя
      сервиса переведено в приглушённый контекст, у сайдбара появился заголовок «СЕРВИСЫ» со счётчиком.
...
- [ ] Тёмная тема Admiral (`LIGHT_THEME` захардкожен в `App.tsx`): нужен выбор темы из конфига.
