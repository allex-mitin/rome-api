...
- [ ] React 18 → 19 — только после проверки совместимости peer-зависимостей
      `swagger-ui-react` и `@asyncapi/react-component`.
...
- [x] **Оболочка переведена на современный вид без разделительных линий.** Убраны все «полоски»:
      вертикальная 4px полоса справа от меню сервисов (это `box-shadow: inset -4px 0 0 0` у
      `VerticalTabMenuContainer`, рисуется из-за `showUnderline` у admiral), 2px акцентная рамка
      шапки и `<Divider/>` под тулбаром спеки. Зоны разделяются фоном и тенью: серая подложка
      `--app-shell`, белые карточки (`src/components/surface.ts`), тонированная полоса тулбара
      (карточки и тонировка позже убраны совсем — см. следующий пункт про плоскую раскладку).
      Отдельная находка: **любая тень под непрозрачной шапкой читается как горизонтальная полоса**
      (box-shadow начинается ровно от края элемента), поэтому тень шапки убрана вообще и граница
      держится только на тоне — проверено скриншотами с диагностической заливкой.
- [x] **Детальная доводка swagger-ui.** Разобрана разметка параметров (по реальному DOM): swagger-ui в неактивном режиме всё равно рендерит `<input disabled>` на каждый параметр (`<input type="text" placeholder="petId" disabled>` и `input[type=file]`) прямо в ячейке описания — на экране это лишние рамки-дубли. Скрыты через `.parameters-col_description input:disabled`; проверено кликом по «Try it out»: те же контролы получают `disabled:false` и остаются видимыми, так что режим не сломан.
      Маркер обязательности: дефолт `.parameter__name.required:after` — `content:"required"` + `top:-6px`,
      а старое правило добавляло `text-transform: uppercase` — рядом с отдельной красной звёздочкой
      получалось «REquIRED». `content` убран, `*` приглушён. `parameter__type` и `parameter__in`
      (`integer($int64)` и `(path)`) свёрнуты в одну компактную строку с разделителем `·`, пустые
      блоки (`:empty`) скрыты. Ссылки info сведены в одну строку, панель SCHEMES стала
      `width: fit-content`, фоны строк операций приглушены — цвет метода остался только в бейдже.
      Отступы всех секций приведены к `--app-gutter`.
- [ ] Встроенный фильтр тегов ищет только по ИМЕНИ тега и регистрозависимо
      (`taggedOps.filter((tagObj, tag) => tag.indexOf(phrase) !== -1)`), а плейсхолдер «Filter by tag»
      захардкожен в `core/containers/filter.jsx`, не в конфиге. Логика подменяется через публичный ключ
      конфига `fn.opsFilter`, но **через `swagger-ui-react` до него не добраться**: обёртка
      деструктурирует фиксированный список пропсов, и `fn` в него не входит. Нужен свой preset/plugin
      либо прямой `swagger-ui`.
- [x] **Настройки рендереров из конфига.** Опции обоих рендереров были захардкожены в
      `SwaggerPage.tsx` и `AsyncApiContainer.tsx`; теперь это только baseline
      (`src/helpers/rendererOptions.ts`), а сверху ложатся секция `renderers` из `settings.yml` и
      `options` конкретной спеки: baseline → `renderers.openapi|asyncapi` → `services[].<spec>.options`.
      Слияние рекурсивное (можно переопределить `show.sidebar`, не повторяя секцию), `null`/пустое
      значение считается «не задано» и дефолты не затирает. Типы опций выведены из самих компонентов
      (`Partial<Omit<ComponentProps<typeof SwaggerUI>, 'url'|'spec'>>` и `ConfigInterface`, который
      пакет AsyncAPI наконец экспортирует из корня) — имена ключей в конфиге совпадают с
      библиотечными. `url`/`spec` из конфига не прокидываются: какую спеку рисовать, решает
      `services[].openapi`. Опции доступны синхронно через `getLoadedSettings()` — лоадеры роутов уже
      дождались `getSettings()`, поэтому рендерер получает настройки при монтировании и swagger-ui не
      переинициализируется.
      Два подводных камня, найденных на ревью и закрытых в том же патче: (1) `swagger-ui-react` собирает
      систему **один раз** (`useEffect(..., [])`) и обновлять опции не умеет — при клиентской навигации
      между сервисами это update, а не remount, поэтому `SwaggerPage` получил `key` по URL спеки, иначе
      опции предыдущего сервиса остались бы у следующего; (2) обёртка передаёт дальше фиксированный
      allow-list пропсов, так что `maxDisplayedTags`/`fn`/`syntaxHighlight` из конфига до swagger-ui не
      доходят (поправлено в README и `settings.yml`), а значения неверной формы для
      `presets`/`plugins`/`requestInterceptor`/`onComplete` отбрасываются санитайзером — иначе
      `[...presets]` внутри обёртки роняет весь роут.
- [x] **Второй проход по оболочке: спека больше не «в окне».** По требованию владельца убрана белая
      карточка со скруглением и тенью, которая отделяла спеку от страницы: теперь поверхность страницы
      и есть поверхность спеки, а сайдбар отделён только пустым пространством (никаких тонированных
      панелей и разделителей). Тулбар стал липким и почти невидимым — `position: sticky` +
      полупрозрачный белый фон с `backdrop-filter: blur(12px)`: контент прокручивается под ним и сам
      создаёт границу, без линии. Переключатель OpenAPI/AsyncAPI уменьшен до `dimension="s"`, у сайдбара
      появился заголовок «СЕРВИСЫ» со счётчиком.
      Единый горизонтальный отступ колонки — `--app-gutter: 32px`: его используют и тулбар, и оба
      рендерера (до этого хардкод 24px давал рассинхрон левых краёв). Скроллером роута стал `Service.tsx`
      (`overflow: auto`), а `Documentation.tsx` перестал скроллиться — без этого `position: sticky`
      у тулбара не работает. Модуль `src/components/surface.ts` удалён: карточек в раскладке больше нет.
      Отдельным заходом переделан порядок в тулбаре (владельцу не понравилась «полоска с кнопками»): имя
      сервиса уехало влево и стало заголовком страницы, а всё, что отвечает на вопрос «какую именно спеку
      я смотрю», собрано справа — переключатель OpenAPI/AsyncAPI, выбор версии и кнопки скачивания.
      Селект версии заменён на компактный чип с поповером (`SpecVersion.tsx`, свой dropdown: клик,
      закрытие по клику вовне и Escape, стрелки по опциям) — admiral'овский `Select` в тулбаре читался
      как поле формы и тянул за собой всю машинку меню. Попутно главный чанк **465 → 392 kB**
      (gzip 145.7 → 125.7).
      Ещё заход по тулбару: бар сделан **непрозрачным** (полупрозрачный с блюром владелец не принял),
      а admiral'овский `ContentSwitcher` заменён своим сегмент-контролом — его внутренности не
      переопределить без боёв с генерируемыми классами, а все остальные контролы в оболочке уже свои.
      Активный сегмент — белая «шайба» с акцентным текстом на сером треке; при одном типе спеки
      переключатель превращается в статичную подпись (переключать нечего).
      Важная находка при проверке непрозрачности: у AsyncAPI-темы на панели контента стоит
      `relative z-10`, поэтому тулбар с `z-index: 4` перекрывался спекой — видно только через
      `document.elementsFromPoint` (визуально это выглядит как «бар не перекрывает контент»).
      Поднят до 50 — верха Tailwind-шкалы, которой пользуется тема рендерера; у swagger такой проблемы нет.
- [x] **Главная страница: витрина форматов + портал по сервисам.** Заголовок с одной фразой «зачем это
      всё», секция «Форматы» — две карточки с логотипами OpenAPI и AsyncAPI, пояснением и ссылкой на сайт
      формата, и секция «Сервисы» — сетка карточек: имя, чипы доступных форматов и число версий; сервис
      без спек помечен «нет спецификаций».
      Логотипы пришлось починить: у `openapi.webp` альфа есть, а у `asyncapi.webp` — нет, фон (#f8fafb)
      вшит в растр, поэтому он рисовался серым квадратом на любой поверхности. ImageMagick в системе
      нет, поэтому фон выбит скриптом на stdlib Python: `dwebp -pam` → alpha=0 у пикселей в пределах 6
      от цвета фона (фон ровный, один цвет по всему периметру) → кроп по bbox → PNG →
      `cwebp -q 92 -alpha_q 100`. Итог: `src/assets/openapi-logo.webp` (440×103, 10 КБ) и
      `asyncapi-logo.webp` (440×121, 15 КБ), оба с альфой. Промежуточные PNG удалены, исходные растры
      оставлены как источник для повторной генерации (в бандл не попадают).
      Сетка `auto-fill minmax(248px, 1fr)` плюс скролл самой страницы рассчитаны на большое число
      сервисов, высота карточек выровнена (`min-height` + `margin-top: auto` у меты — проверено:
      у всех восьми ровно 104px).
- [x] **Отказ от UI-кита AdmiralDS и от шрифта VTB Group UI.** Продукт может стать коммерческим, а
      `@admiral-ds/fonts` — это корпоративный гарнитур ВТБ без отдельной лицензии на шрифтовое ПО
      (MIT-файл пакета лицензирует код, а не шрифт), при этом файлы физически ехали в поставку:
      `VTBGroupUI-SemiBold.ttf` 343K + `-Medium.otf` 94K + `-Regular.otf` 93K ≈ 530 KB. Удалены все три
      пакета (`react-ui`, `icons`, `fonts`) вместе с `ThemeProvider`, `FontsVTBGroup` и `LIGHT_THEME`.
      Вместо них: свои `src/components/icons.tsx` (6 знаков, штрих 1.6px, `currentColor`), `Spinner.tsx`
      (кольцо на CSS, `prefers-reduced-motion`), `IconButton.tsx`; типографика шапки воспроизведена
      точь-в-точь (20/28/600 и 14/20/400 + opacity), `--app-font-family` переведён на системный стек.
      Итог: статика −572 KB, шрифтовых файлов в сборке 0, главный чанк 386.8 → 351.9 kB (`npm ls`
      пуст, строки `admiral` нет ни в `src`, ни в бандле). Компромисс: шкалу типографики дизайн-системы
      мы больше не получаем — значения перенесены руками, при добавлении новых текстов надо
      выдерживать те же размеры самому.
- [x] **Единое состояние «спеку не удалось показать».** Раньше битая спека выглядела по-разному:
      swagger-ui рисовал свою английскую простыню «Unable to render this definition…», а AsyncAPI
      оставлял пустую страницу (причина была спрятана в свёрнутой панели валидации). Введён флаг
      `fatal` у диагностик (`src/helpers/specValidation.ts`): не загрузилось / не распарсилось / это не
      OpenAPI-документ — фатально; «рисуется, но с проблемами» — нет. Фатальные случаи показывают новый
      `src/components/SpecFailure.tsx` (иконка, причина, подсказка про `settings.yml` и SPA-фолбэк,
      ссылка на сам адрес спецификации, список остальных замечаний) вместо рендерера — swagger-ui в этом
      случае даже не монтируется, чтобы не тянуть заведомо битый документ. Проверено скриншотами:
      `missing/openapi` и `missing/asyncapi` показывают это состояние, `invalid/openapi` по-прежнему
      рисует спеку вместе с панелью проблем (то есть ложных срабатываний нет).
      Попутно исправлен давний баг: `ValidationPanel` — flex-элемент рядом с рендерером, а его
      `overflow: hidden` обнуляет автоматический минимальный размер flex-элемента, поэтому при спеке
      выше вьюпорта панель сжималась до 2px (замер: было 2px, стало 39.5px) — добавлен `flex: 0 0 auto`.
- [ ] Third-party notices: в поставке нет копий лицензий и NOTICE, а Apache-2.0 (§4) этого требует
      (у `swagger-ui` NOTICE есть). Нужен файл `THIRD-PARTY-LICENSES` рядом со статикой.
- [x] **Фильтр по названию сервиса.** `TabMenuVertical` в сайдбаре заменён на собственный список
      (`Navigator.tsx`): поле фильтра + независимо скроллящийся список, поэтому большое число
      сервисов больше не ломает раскладку (у admiral для этого было overflow-меню). Выбранный
      сервис — акцентная пилюля из `branding.accentColor` (пробрасывается CSS-переменной
      `--app-accent`). Побочно главный чанк избавился от импорта компонентов admiral-табов.
- [x] **Большие спеки.** `SwaggerUI` получил `filter` и `defaultModelsExpandDepth={-1}`: список
      операций сужается встроенным полем, а секция Models не рендерит все схемы сразу.
- [x] **Раздельные CSS рендереров.** `.swagger-ui` правила вынесены в `src/assets/css/openapi.css`,
      AsyncAPI — в `asyncapi.css` (подключаются в `index.tsx` после стилей пакетов). У AsyncAPI
      они пишутся с повышенной специфичностью (`.aui-root .border-b`): пакет отдаёт предсобранные
      unlayered Tailwind-утилиты, которые перебивают наши слои.
- [x] **Брендинг из конфига.** Секция `branding` в `settings.yml` (типы — `Branding` в `src/types.ts`,
      значения по умолчанию и резолв — `src/helpers/branding.ts`): название, подпись, логотип (URL),
      высота логотипа, ссылка с шапки, фон шапки, цвет текста, акцентный цвет, заголовок вкладки и
      фавикон. Всё необязательно, поэтому переопределяется только то, что нужно (проверено в Node:
      пустая секция даёт дефолты, `logoHeight: "48"` приводится к числу, `subtitle: ""` скрывает строку).
      Ключевое: файл читается в браузере и лежит рядом со сборкой, поэтому перебрендирование не требует
      пересборки фронтенда. Логотип по внешнему URL подставляется как `<img>`, а при 404 откатывается
      на вшитый — шапка не показывает битую картинку. В `deploy/nginx.conf.example` под это добавлены
      `/branding/` (`^~`, иначе regex-правило кэширует логотип на год) и `= /settings.js`, который
      раньше попадал под правило immutable-кэша.
      Заодно шапка перевёрстана (подпись под названием, разделитель акцентным цветом, тень), а
      `settings.yml`/`settings.js` теперь фетчатся один раз на загрузку страницы вместо трёх
      (общий `settingsPromise`): `Services()`, `getService()` и брендинг читают один и тот же результат.
      `TYPES_EXIT=0`, `LINT_EXIT=0`, `BUILD_EXIT=0`.
      **Требуется проверка в браузере** — внешний вид шапки и подмена логотипа/фавикона из конфига.
- [ ] Убрать `classnames` из зависимостей: после переезда `LoadingSpec` на styled-components он больше не используется.
- [ ] Адаптив оболочки: сайдбар фиксированной ширины 268px (216px на узких экранах) — на телефоне
      нужен сворачиваемый вариант.
- [ ] Тёмная тема: сейчас светлая палитра захардкожена в CSS-переменных (`--app-*`), нужен выбор
      темы из конфига. (Пункт про тёмную тему Admiral снят — AdmiralDS из проекта выведен.)
